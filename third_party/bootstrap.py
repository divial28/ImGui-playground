...
def cloneRepository(type, url, target_name, revision = None, try_only_local_operations = False):
    target_dir = escapifyPath(os.path.join(THIRD_PARTY_DIR, target_name))
...
def extractFile(filename, target_dir):
    ...
    if extension == ".zip" or extension == "":
        ...
        extract_dir_abs = os.path.join(THIRD_PARTY_DIR, extract_dir_local)
    ...
    extract_dir_abs = os.path.join(THIRD_PARTY_DIR, extract_dir)
...
def downloadAndExtractFile(url, download_dir, target_dir_name, sha1_hash = None, force_download = False, user_agent = None):
    ...
    extractFile(target_filename, os.path.join(THIRD_PARTY_DIR, target_dir_name))
...
def applyPatchFile(patch_name, dir_name, pnum):
    ...
    arguments = "-d " + os.path.join(THIRD_PARTY_DIR, dir_name) + " -p" + str(pnum) + " < " + os.path.join(patch_dir, patch_name)
    argumentsBinary = "-d " + os.path.join(THIRD_PARTY_DIR, dir_name) + " -p" + str(pnum) + " --binary < " + os.path.join(patch_dir, patch_name)
...
def printOptions():
        print("--------------------------------------------------------------------------------")
        print("Downloads external libraries.")
...
def main(argv):
    global BASE_DIR, THIRD_PARTY_DIR, ARCHIVE_DIR, DEBUG_OUTPUT, FALLBACK_URL, USE_TAR, USE_UNZIP
    ...
    for library in data:
        ...
        lib_dir = os.path.join(THIRD_PARTY_DIR, name)
                ...
                if src_type == "sourcefile":
                    ...
                    try:
                        ...
                        if create_repo_snapshots:
                            ...
                            repo_dir = os.path.join(THIRD_PARTY_DIR, name)
